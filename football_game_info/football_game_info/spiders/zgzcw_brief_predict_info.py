...
class ZgzcwBriefPredictInfoSpider(scrapy.Spider):
    name = 'zgzcw_brief_predict_info'
    ...
    def start_requests(self):
        ...
        fork_date = datetime.datetime.now() + datetime.timedelta(days = -1)
    ...
    def parse(self, response):
        ...
        for tr in trs:
            ...
            if status is None or status.strip() != u'未':
                continue
            ...
            item = FSpiderLotteryPredictInfo(
                matchid=int(matchid),
                issue=0,
                status="未开始",
                game=game,
